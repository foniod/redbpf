...
/*!
A module for generating C source code that defines internal structs and enums
of the Linux kernel.

Generating C source code using BTF of vmlinux. All data structures even not
exported in the Linux kernel headers can be dumped into a C header file.

*NOTE* BTF does not record macro constants that are defined by `#define` syntax
in the BTF section of vmlinux. So macro constants are not generated by BTF of
vmlinux. But `type_gen` adds common macro constants into rust bindings by
including some system C header files and then generating rust bindings from
them.
*/
...
impl VmlinuxBtfDump {
    ...
    /// BTF data out of it to use for target BTF.
    ...
    /// Try to load BTF data out of given ELF file
    ...
    /// Dump BTF types in C into `outfile` including all the necessary
    /// dependent types.
